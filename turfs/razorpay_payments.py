...
def credentials(modelname: str):
    if modelname.upper() == "TEST":
        cred = Test_credentials.objects.values("key_id", "secret").get(
            active_status="YES"
        )
        print("cred :: ", cred)
        if cred:
            return {"key_id": cred["key_id"], "secret": cred["secret"], "stat": "Ok"}
        else:
            return {"key_id": "", "secret": "", "stat": "Not Ok"}
    elif modelname.upper() == "LIVE":
        cred = Live_credentials.objects.values("key_id", "secret").get(
            active_status="YES"
        )
        print("cred :: ", cred)
        if cred:
...
def create_client_session(KEY_ID: str, KEY_SECRET: str):
    print("KEY_ID :: ", KEY_ID)
...
def get_customer_by_email(email, client):
    try:
        ...
        return {"error": str(e), "stat": "Not OK"}
...
def initiate_penny_drop(
    fund_account_id, client, amount=100, currency="INR", mode="IMPS"
):
    try:
        payout_data = {
...
#             return {"data": check_customer["customer"]}
...
#                 )
#                 if fund_acc["stat"] == "Ok":
#                     initi_penny_drop = initiate_penny_drop(
#                         fund_acc["fund_account"]["id"], session
#                     )
#                     if initi_penny_drop["stat"] == "Ok":
#                         payout_status = get_payout_status(
#                             initi_penny_drop["payout"]["id"], session
#                         )
...
#         #     }
#     except Exception as e:
#         return {"stat": "Not Ok", "error": str(e), "msg": "syntax error 1"}


def main_func(using_cred: str,clientname: str,clientemail: str,contact: str):
    resp1 = create_customer(using_cred,clientname,clientemail,contact)
    return resp1
